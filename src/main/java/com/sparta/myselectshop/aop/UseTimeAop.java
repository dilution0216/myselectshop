...
@Slf4j(topic = "UseTimeAop") // log 찍기 위해서 설정, 구분하기 위해 topic 도 줌
@Aspect
@Component
...
public class UseTimeAop {
    ...
    // 우리가 만드는 기능은시작 전, 후 시간을 찍어서 수행 시간 계산해야 하기 때문에 @Around 로 전후 다 설정
    // product folder naver 중 하나라도 수행하면 api 수행 시간 누적시킬거기 때문에 세개 다 작성
    @Around("product() || folder() || naver()")
    public Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
        // 측정 시작 시간
        long startTime = System.currentTimeMillis(); // 측정시간 찍어 놓은 후

        try {
            // 핵심기능 수행
            Object output = joinPoint.proceed(); // 기능의 메서드 수행
            return output;
        } finally {
            // 측정 종료 시간
