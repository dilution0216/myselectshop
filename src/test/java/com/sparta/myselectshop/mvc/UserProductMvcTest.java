...
class UserProductMvcTest {
    ...
    // UserController 를 사용하기 위해선 UserService,FolderService,Kakao Service 빈 주입이 필요함
    // 그래서 가짜 빈을 주입해준다.
    // UserController 를 보면, 밑에 3가지를 주입 받아주는 생성자가 필요함
    // private final UserService userService;
    // private final FolderService folderService;
    // private final KakaoService kakaoService;
    // 주입하여 주기 위해서 가짜 빈을 만들어 주는 것
    @MockBean
    ...
    void test1() throws Exception {
        // when - then
        mvc.perform(get("/api/user/login-page")) // 어떤 방식인지 (get,post등) 선택
                .andExpect(status().isOk()) // status code 무엇인지 isOk() 에측 해주기
    ...
    void test2() throws Exception {
        ...
        // when - then
        mvc.perform(post("/api/user/signup") // post 쓰는 이유: body 에 넣어줘야 하니까
